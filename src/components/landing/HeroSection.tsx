...
import { ArrowRight, Play, Bell, Calendar, PieChart, Sparkles } from 'lucide-react';
  ...
  return (
    <section className="relative pt-32 pb-20 lg:pt-40 lg:pb-32 bg-gradient-hero overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-40 left-[15%] w-2 h-2 bg-accent rounded-full animate-bounce-subtle opacity-60" style={{ animationDelay: '0.5s' }} />
        <div className="absolute bottom-40 right-[30%] w-4 h-4 bg-primary/60 rounded-full animate-bounce-subtle" style={{ animationDelay: '1s' }} />
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className={cn(
            ...
            {t.hero.title}{' '}
            <span className="text-gradient animate-gradient bg-gradient-to-r from-primary via-accent to-primary bg-[length:200%_auto]">{t.hero.titleHighlight}</span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16 animate-fade-up stagger-3">
            <Link to="/register">
              <Button variant="hero" size="xl" className={cn("ai-glow", language === 'bn' && "font-bangla")}>
                {t.hero.cta}
                <ArrowRight className="w-5 h-5" />
              </Button>
            </Link>
            <Button variant="hero-outline" size="xl" className={cn("hover-lift", language === 'bn' && "font-bangla")}>
            ...
            <div className="relative bg-card rounded-2xl shadow-xl border border-border overflow-hidden">
              ...
              <div className="p-6 bg-background">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  {[
                    { icon: <Calendar className="w-5 h-5 text-primary" />, label: "12", sublabel: "Subscriptions" },
                    { icon: <Bell className="w-5 h-5 text-warning" />, label: "3", sublabel: "Renewals Soon" },
                    { icon: <PieChart className="w-5 h-5 text-accent" />, label: "$214", sublabel: "Monthly Cost" },
                  ].map((stat, i) => (
                    <div key={i} className="p-4 rounded-xl bg-card border border-border hover:border-primary/30 transition-all duration-300">
                ...
                <div className="h-32 rounded-xl bg-gradient-to-r from-muted/30 via-muted/50 to-muted/30 border border-border/50 animate-shimmer" style={{ backgroundImage: 'linear-gradient(90deg, transparent, hsl(var(--primary) / 0.05), transparent)' }} />
              </div>
            </div>
          </div>
        </div>
